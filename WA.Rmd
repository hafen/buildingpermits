---
title: "Housing Permit Data Summary: WA"
author: Ryan Hafen
output:
  packagedocs:
    toc: true
...
navpills: |
  ...
  <a id="study-drop" href="#" class="dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" role="button" aria-expanded="false">Dataset (WA)<span class="caret"></span></a>
  ...
  <li class='active'><a href='WA.html'>WA</a></li>
  </ul>
  </li>
---
...
library(rbokeh)
library(knitr)
load("/var/folders/n4/6ztyqms165s3r_4n001n1y6m0000gn/T//RtmpvNpZPH/file421b6cc1cb3e")
```
...
## project_address.latitude ##

### Distribution ###

```{r, echo=FALSE, message=FALSE, results='asis'}
vr <- di$var_summ[["project_address.latitude"]]
vr$artifacts$fg
```
...
## project_address.human_address ##

### Distribution ###

```{r, echo=FALSE, message=FALSE, results='asis'}
vr <- di$var_summ[["project_address.human_address"]]
vr$artifacts$fg
```
...
## project_address.longitude ##

### Distribution ###

```{r, echo=FALSE, message=FALSE, results='asis'}
vr <- di$var_summ[["project_address.longitude"]]
vr$artifacts$fg
```
